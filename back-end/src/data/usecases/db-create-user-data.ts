...
export class DbCreateUserData implements CreateUserData {
  ...
  async create (params: CreateUserData.Params): Promise<UserModelDto> {
    const { cpf, email, name, registration_number, admin, password } = params
    ...
    const isRegisterNumberAvailable = await this._loadUserByRegisterNumberRepository.loadByRegisterNumber(registration_number)
